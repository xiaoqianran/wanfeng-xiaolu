...
function renderJournal() {
  ...
  function drawWeather(ctx, w, h, themeId, time) {
    // Hand-crafted atmospheres for core themes; batch path themes use generic motes.
    // CRITICAL: never reintroduce thousands of nested else-if — browsers hit parse stack limits.
    if (themeId === "riverside") {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.quadraticCurveTo(x + Math.sin(time * 0.02 + i) * 6, h * 0.5, x, h);
        ctx.stroke();
      }
    } else if (themeId === "rain_eaves" || themeId === "rain_garden") {
      ctx.strokeStyle = "rgba(180,200,220,0.4)";
      ctx.lineWidth = 1;
      for (let i = 0; i < 20; i++) {
        const x = ((i * 61 + time * 1.2) % (w + 10)) - 5;
        const y = (i * 29 + time * 2) % h;
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.lineTo(x - 1.5, y + 12);
        ctx.stroke();
      }
    } else if (themeId === "lantern_street" || themeId === "night_market" || themeId === "snow_lantern") {
      for (let i = 0; i < 6; i++) {
        const x = 30 + i * (w / 6);
    ...
    } else if (themeId === "harbor" || themeId === "tide_pool" || themeId === "star_dock") {
      ctx.strokeStyle = "rgba(160,200,220,0.3)";
      for (let i = 0; i < 10; i++) {
        const y = h * 0.55 + Math.sin(time * 0.03 + i) * 4 + i * 3;
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.quadraticCurveTo(w * 0.5, y + 6, w, y);
        ctx.stroke();
      }
    } else if (themeId === "meadow" || themeId === "flower_alley" || themeId === "firefly_field") {
      ctx.fillStyle = themeId === "firefly_field" ? "rgba(220,255,160,0.55)" : "rgba(180,220,120,0.35)";
      for (let i = 0; i < 16; i++) {
        const x = (i * 83 + time * 0.15) % w;
        const y = h * 0.35 + ((i * 17) % 40) + Math.sin(time * 0.04 + i) * 3;
        ctx.globalAlpha = 0.35 + 0.35 * Math.sin(time * 0.06 + i);
        ctx.beginPath();
        ctx.arc(x, y, themeId === "firefly_field" ? 1.8 : 2.4, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.globalAlpha = 1;
    } else if (themeId === "plum_grove" || themeId === "rose_lane" || themeId === "orchard_dusk") {
      ctx.fillStyle = "rgba(220,140,160,0.4)";
      for (let i = 0; i < 14; i++) {
        const x = 24 + (i * 52 + Math.sin(time * 0.02 + i) * 2) % (w - 40);
        const y = h * 0.28 + ((i * 13) % 42);
        ctx.beginPath();
        ctx.ellipse(x, y, 5, 3.5, 0.1 * (i % 3), 0, Math.PI * 2);
        ctx.fill();
      }
    } else if (themeId === "mist_bridge" || themeId === "cloud_pass" || themeId === "dawn_bridge") {
      for (let i = 0; i < 4; i++) {
        const x = ((time * 0.2 + i * 90) % (w + 120)) - 60;
        const y = h * 0.4 + i * 12;
        const grad = ctx.createRadialGradient(x, y, 10, x, y, 70);
        grad.addColorStop(0, "rgba(240,245,255,0.22)");
      ...
      // Generic soft motes for thousands of batch path themes
      let hash = 0;
      const id = String(themeId || "");
      for (let i = 0; i < id.length; i++) hash = (hash * 31 + id.charCodeAt(i)) >>> 0;
      const palette = [
        "rgba(180,100,140,0.5)", "rgba(100,160,100,0.5)", "rgba(200,180,80,0.5)", "rgba(100,130,180,0.5)",
      ...
      const n = 12 + (hash % 6);
      for (let i = 0; i < n; i++) {
        const x = 24 + (i * 52 + Math.sin(time * 0.02 + i + hash) * 2) % (w - 40);
        const y = h * 0.32 + ((i * 13 + hash) % 42);
        ctx.beginPath();
        ctx.ellipse(x, y, 5, 3.5, 0.1 * (i % 3), 0, Math.PI * 2);
        ctx.fill();
      }
...
(function wireWateringCan() {
  ...
  function scoreDrink() {
    ...
    if (season === "summer" && (baseDef.id === "soda" || baseDef.id === "berry_soda" || baseDef.id === "lemon" || (flavorDef && flavorDef.tags && (flavorDef.tags.indexOf("清爽") >= 0 || flavorDef.tags.indexOf("果香") >= 0)))) {
    ...
    if (season === "winter" && (baseDef.id === "tea" || baseDef.id === "honey_water" || baseDef.id === "floral_tea" || (flavorDef && (flavorDef.id === "tea_leaf" || flavorDef.id === "yuzu" || flavorDef.id === "ginger" || flavorDef.id === "honey" || (flavorDef.tags && (flavorDef.tags.indexOf("温热") >= 0 || flavorDef.tags.indexOf("草本") >= 0)))))) {
